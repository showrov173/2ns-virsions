...
const allowedDomains = ["showrov173.github.io", "localhost", "127.0.0.1"];
if (!allowedDomains.includes(window.location.hostname)) {
    document.body.innerHTML = "<div style='display:flex; justify-content:center; align-items:center; height:100vh; background:#0f172a; color:#ef4444; font-family:sans-serif; text-align:center; flex-direction:column;'><h1 style='font-size:50px;'>🚨</h1><h2>Unauthorized Use!</h2><p>This software is protected by copyright.</p></div>";
    window.stop(); 
}
...
let waNumber = "";
let currencySymbol = "৳"; // Settings theke update hobe
...
document.addEventListener("DOMContentLoaded", () => {
    const shopId = localStorage.getItem('smartpos_shop_id');
    const landing = document.getElementById('saas-landing');
    ...
    if(posContainer) posContainer.style.setProperty('display', 'none', 'important');

    if (shopId && shopId !== "null") {
        if(posContainer) posContainer.style.setProperty('display', 'flex', 'important');
        if(skeleton) skeleton.style.display = 'none';
        
        currentShopID = shopId;
        loadLocalData(shopId);
        finalizeUIRender();
    } else {
...
function showLoginPage() {
    ...
    if(landing) { landing.classList.remove('hidden'); landing.style.setProperty('display', 'flex', 'important'); }
}
...
function activateLicense() {
    ...
    // Check if it's the offline fallback password
    ...
    db.collection('licenses').doc(inputKey).get().then((doc) => {
        ...
        if(doc.exists && doc.data().isActive) {
            ...
            document.getElementById('saas-landing').style.setProperty('display', 'none', 'important');
            loadDataAndSync(currentShopID); 
            showDashboard();
...
// ================= ADMIN FUNCTIONS (Secure Firebase Check) =================
function checkAdminAccess() {
    let pass = prompt("🔒 Enter Master Admin Password:"); 
    if (!pass || pass.trim() === "") return;

    if (!db) {
        let localPass = localStorage.getItem('smartpos_admin_pass') || "ADMIN-PRO-MAX";
        if (pass === localPass) { openAdminPanel(); } else { alert("❌ Wrong Password! (Offline Mode)"); }
        return;
    }

    db.collection('system').doc('config').get().then((doc) => {
        let realAdminPass = "123456"; 
        if (doc.exists && doc.data().adminPassword) { realAdminPass = doc.data().adminPassword; }
        
        if (pass === realAdminPass) {
            openAdminPanel();
        } else { 
        ...
        let localPass = localStorage.getItem('smartpos_admin_pass') || "ADMIN-PRO-MAX";
        if (pass === localPass) { openAdminPanel(); } else { alert("❌ Wrong Password! (Network Error)"); }
    });
}
...
function openAdminPanel() {
    ...
    loadMasterDatabase(); loadPlans(); loadAdminStats(); loadGlobalSettings(); loadSupportTickets(); 
    if(typeof loadMaintenanceStatus === 'function') loadMaintenanceStatus();
    alert("Welcome Boss! 🚀");
}

function changeAdminPassword() {
    let pass = prompt("🔒 বর্তমান পাসওয়ার্ড দিন:");
    db.collection('system').doc('config').get().then((doc) => {
        let realAdminPass = doc.exists && doc.data().adminPassword ? doc.data().adminPassword : "ADMIN-PRO-MAX";
        if (pass === realAdminPass) {
            let newPass = prompt("🔑 নতুন পাসওয়ার্ডটি লিখুন:");
            if (newPass && newPass.trim() !== "") { 
                db.collection('system').doc('config').set({ adminPassword: newPass.trim() }, { merge: true }).then(()=> {
                    localStorage.setItem('smartpos_admin_pass', newPass.trim());
                    alert("✅ পাসওয়ার্ড সফলভাবে পরিবর্তন করা হয়েছে!"); 
                });
            } else { alert("❌ পাসওয়ার্ড খালি রাখা যাবে না!"); }
        } else { alert("❌ বর্তমান পাসওয়ার্ডটি ভুল!"); }
    });
}

function logoutAdmin() {
    localStorage.removeItem('is_super_admin'); 
    let adminPanel = document.getElementById('super-admin-panel'); if(adminPanel) { adminPanel.classList.add('hidden'); adminPanel.style.display = 'none'; }
    let posContainer = document.querySelector('.pos-container');
    if(posContainer) posContainer.style.setProperty('display', 'flex', 'important');
    checkSubscription();
}
...
function listenToMaintenanceMode() {
    ...
    db.collection('system').doc('maintenance').onSnapshot((doc) => {
        if(doc.exists) {
            ...
            let isMasterAdmin = (localStorage.getItem('is_super_admin') === 'true');
...
function switchDashboardView(viewMode) {
    ...
    // Setting elements
    document.getElementById('dash-today-profit').innerText = `${currencySymbol} ${tProfit.toFixed(2)}`; 
    document.getElementById('dash-today-net-profit').innerText = `${currencySymbol} ${tNetProfit.toFixed(2)}`; 
    document.getElementById('dash-today-sell').innerText = `${currencySymbol} ${tSell.toFixed(2)}`; 
    document.getElementById('dash-today-cost').innerText = `${currencySymbol} ${tCost.toFixed(2)}`; 
    document.getElementById('dash-today-due-col').innerText = `${currencySymbol} ${tDueCol.toFixed(2)}`; 
    document.getElementById('dash-today-cash').innerText = `${currencySymbol} ${((tSell + tDueCol) - tCost).toFixed(2)}`; 
    document.getElementById('dash-today-bills').innerText = `${countBills} টি`; 
    document.getElementById('dash-month-sell').innerText = `${currencySymbol} ${mSell.toFixed(2)}`; 
    document.getElementById('dash-month-profit').innerText = `${currencySymbol} ${mProfit.toFixed(2)}`; 
    document.getElementById('dash-month-cost').innerText = `${currencySymbol} ${mCost.toFixed(2)}`; 
    document.getElementById('dash-month-net-profit').innerText = `${currencySymbol} ${mNetProfit.toFixed(2)}`; 
    document.getElementById('dash-total-due').innerText = `${currencySymbol} ${totalMarketDue.toFixed(2)}`;
...
function updateDashboardAlerts() {
    let transHtml = ''; salesHistoryDB.slice(0, 5).forEach(s => { transHtml += `<li><div><b>${s.customer}</b><br><small style="font-size:10px;">${s.date.split(',')[0]}</small></div><b style="color:#10b981;">+ ${currencySymbol}${s.total.toFixed(0)}</b></li>`; }); document.getElementById('dash-recent-trans').innerHTML = transHtml || '<li>No transactions</li>';
    let dueHtml = ''; let topDebtors = [...customersDB].filter(c => c.due > 0).sort((a, b) => b.due - a.due).slice(0, 5); topDebtors.forEach(c => { dueHtml += `<li><span><i class="fa-solid fa-user"></i> ${c.name}</span><b style="color:#ef4444;">${currencySymbol}${c.due.toFixed(0)}</b></li>`; }); document.getElementById('dash-due-reminder').innerHTML = dueHtml || '<li>All clear!</li>';
    let stockHtml = ''; let lowStocks = [...productsDB].filter(p => p.stock <= 5).slice(0, 5); lowStocks.forEach(p => { stockHtml += `<li><span><i class="fa-solid fa-box"></i> ${p.name}</span><b style="color:#ef4444;">${p.stock} ${p.unit||'Pcs'}</b></li>`; }); document.getElementById('dash-low-stock').innerHTML = stockHtml || '<li>Stock is healthy!</li>';
    let topProdHtml = ''; [...productsDB].filter(p=>p.isTop).slice(0,5).forEach(p => { topProdHtml += `<li><span>🔥 ${p.name}</span><b style="color:#3b82f6;">${currencySymbol}${p.sell}</b></li>`; }); document.getElementById('dash-top-products').innerHTML = topProdHtml || '<li>No top products</li>';
}

function initChart() { 
    const ctx1 = document.getElementById('myChart'); if(!ctx1) return; 
    if (salesChart) { salesChart.destroy(); }
    let config = { type: 'line', data: { labels: ['6 Days Ago','5 Days Ago','4 Days Ago','3 Days Ago','2 Days Ago','Yesterday','Today'], datasets: [{ label: `Sales Trend (${currencySymbol})`, data: [0,0,0,0,0,0,0], borderColor: '#3b82f6', backgroundColor: 'rgba(59, 130, 246, 0.2)', borderWidth: 2, fill: true, tension: 0.4 }] }, options: { responsive: true, maintainAspectRatio: false } }; 
...
function renderAllTables() {
    const prodBody = document.getElementById('prod-list-body'); let lowStockCount = 0; let totalInventoryValue = 0;
    if(prodBody) {
        let prodHtml = ''; 
        productsDB.forEach((p) => {
            if(p.stock <= 5) lowStockCount++;
            totalInventoryValue += (p.stock * p.buy); // 🔥 Inventory Value Calc
            ...
            prodHtml += `<tr><td style="display: flex; align-items:center; gap:10px;"><img src="${imgSrc}" style="width:40px; height:40px; border-radius:5px; object-fit:cover; border: 1px solid #e2e8f0;"><b>${p.name}</b></td><td>${stockStatus}</td><td>${currencySymbol} ${p.buy}</td><td>${currencySymbol} ${p.sell}</td><td class="no-print"><button class="btn btn-primary btn-small" onclick="editProduct('${p.name}')"><i class="fa-solid fa-pen"></i></button> <button class="btn btn-danger btn-small" onclick="deleteProduct('${p.name}')"><i class="fa-solid fa-trash"></i></button></td></tr>`;
        });
        ...
        if(document.getElementById('stat-total-value')) document.getElementById('stat-total-value').innerText = `${currencySymbol} ${totalInventoryValue.toLocaleString()}`;
    }
    ...
    if(cusBody) {
        ...
        customersDB.forEach((c) => {
            ...
            cusHtml += `<tr><td onclick="viewCustomerLedger('${c.name}')" style="cursor:pointer; display: flex; align-items:center; gap:12px;"><img src="${imgSrc}" style="width:40px; height:40px; border-radius:50%; object-fit:cover; border: 1px solid #e2e8f0; background: #fff;"><div><b>${c.name}</b><br><small style="color:#64748b;">${c.phone}</small></div></td><td>${catTag}</td><td style="color:#ef4444; font-weight:bold;">${currencySymbol} ${c.due.toFixed(2)}</td><td class="no-print"><button class="btn btn-success btn-small" onclick="sendDueWhatsApp('${c.phone}', '${c.name}', ${c.due})" title="Send Reminder"><i class="fa-brands fa-whatsapp"></i></button> <button class="btn btn-primary btn-small" onclick="editCustomer('${c.name}')"><i class="fa-solid fa-pencil"></i></button> <button class="btn btn-danger btn-small" onclick="deleteCustomer('${c.name}')"><i class="fa-solid fa-trash"></i></button></td></tr>`;
        });
...
function viewCustomerLedger(name) { activeLedgerCus = name; let c = customersDB.find(cus => cus.name === name); if(!c) return; document.getElementById('history-cus-name').innerText = c.name; document.getElementById('ledger-tot-due').innerText = `${currencySymbol} ${c.due.toFixed(2)}`; let tbody = document.getElementById('history-list-body'); tbody.innerHTML = ''; let totalBuy = 0; salesHistoryDB.forEach(s => { if(s.customer === name) { totalBuy += s.total; tbody.innerHTML += `<tr><td>${s.date.split(',')[0]}</td><td style="color:#1e293b; font-weight:bold;">Bill (${s.invoice})</td><td style="color:#ef4444;">- ${currencySymbol}${s.total.toFixed(2)}</td></tr>`; } }); dueCollectionHistoryDB.forEach(col => { if(col.customer === name) { tbody.innerHTML += `<tr><td>${col.date.split(',')[0]}</td><td style="color:#1e293b; font-weight:bold;">Payment</td><td style="color:#10b981;">+ ${currencySymbol}${col.amount.toFixed(2)}</td></tr>`; } }); document.getElementById('ledger-tot-buy').innerText = `${currencySymbol} ${totalBuy.toFixed(2)}`; document.getElementById('historyModal').classList.remove('hidden'); }
...
function selectCustomer(name, dueAmt) { document.getElementById('cus-search-billing').value = name; document.getElementById('cus-dropdown-list').classList.add('hidden'); currentCustomerDue = dueAmt || 0; document.getElementById('bill-prev-due').innerText = `${currencySymbol} ${currentCustomerDue.toFixed(2)}`; calculateCartTotal(); }
function walkIn() { document.getElementById('cus-search-billing').value = "Walk-in"; document.getElementById('cus-dropdown-list').classList.add('hidden'); currentCustomerDue = 0; document.getElementById('bill-prev-due').innerText = `${currencySymbol} 0`; calculateCartTotal(); }
    ...
    if(cartItems.length === 0) {
        ...
        cartItems.forEach((item, i) => { 
            ...
            <tr style="border-bottom:1px solid #f1f5f9;">
                ...
                <td style="font-weight:bold; color:#3b82f6;">${currencySymbol}${total.toFixed(2)}</td>
...
function calculateCartTotal() {
    document.getElementById('bill-subtotal').innerText = `${currencySymbol} ${cartTotal.toFixed(2)}`; let chk = document.getElementById('chk-prev-due'); let prevDueToAdd = (chk && chk.checked) ? currentCustomerDue : 0; let discInput = document.getElementById('bill-discount'); let discount = parseFloat(discInput ? discInput.value : 0) || 0;
    let grandTotal = Math.max(0, (cartTotal + prevDueToAdd) - discount); document.getElementById('bill-grand-total').innerText = `Total: ${currencySymbol} ${grandTotal.toFixed(2)}`;
...
function finalizeSale(action) {
    ...
    let grandTotal = parseFloat(document.getElementById('bill-grand-total').innerText.replace(`Total: ${currencySymbol} `, '')) || 0; 
    ...
    let printItemsHtml = `<table style="width:100%; border-collapse: collapse; margin: 15px 0; font-size: 13px; text-align: left;"><tr style="border-bottom: 2px solid #333;"><th style="padding:5px 0; background:transparent;">বিবরণ</th><th style="background:transparent;">পরিমাণ</th><th style="text-align:right; background:transparent;">মোট (${currencySymbol})</th></tr>`;
    let waItemsText = ``;
    
    cartItems.forEach(item => {
        let itemTotal = item.qty * item.price;
        printItemsHtml += `<tr style="border-bottom: 1px dotted #ccc;"><td style="padding:5px 0;">${item.name}<br><small>@${currencySymbol}${item.price}</small></td><td>${item.qty}</td><td style="text-align:right;">${itemTotal.toFixed(2)}</td></tr>`;
        waItemsText += `▪ ${item.name} (${item.qty} x ${item.price}) = ${currencySymbol}${itemTotal.toFixed(2)}\n`;
    });
    printItemsHtml += `</table>`;
    
    let waMessage = `*${shopName}*\n📅 ${dateObj.toLocaleDateString()}\n🧾 Inv: ${invoiceNo}\n👤 Customer: ${cusName}\n\n*-- আইটেমসমূহ --*\n${waItemsText}\n*Subtotal:* ${currencySymbol}${cartTotal.toFixed(2)}\n` + (pDue > 0 ? `*Prev. Due:* ${currencySymbol}${pDue.toFixed(2)}\n` : ``) + (discount > 0 ? `*Discount:* -${currencySymbol}${discount.toFixed(2)}\n` : ``) + `-------------------\n*Grand Total: ${currencySymbol}${grandTotal.toFixed(2)}*\n*Paid:* ${currencySymbol}${paidAmt.toFixed(2)}\n*Current Due:* ${currencySymbol}${currentDue.toFixed(2)}\n\nধন্যবাদ! আবার আসবেন।`;
    ...
    if(action === 'print') { 
        let finalInvoiceHTML = `<div style="font-family: 'Hind Siliguri', sans-serif; color: #000; max-width: 300px; margin: 0 auto; padding: 10px;"><h2 style="text-align:center; margin-bottom: 5px; color: #000; font-size: 22px;">${shopName}</h2><div style="text-align:center; font-size:12px; margin-bottom: 15px;">মোবাইল: ${waNumber || "N/A"}</div><div style="font-size:13px; border-bottom:1px solid #000; padding-bottom:5px; margin-bottom: 5px;"><b>Inv No:</b> ${invoiceNo}<br><b>Date:</b> ${dateObj.toLocaleString('en-US', {hour12:true})}<br><b>Customer:</b> ${cusName}</div>${printItemsHtml}<div style="text-align:right; font-size:14px; margin-top: 10px;"><p style="margin:2px 0;">Subtotal: ${currencySymbol}${cartTotal.toFixed(2)}</p>${pDue > 0 ? `<p style="margin:2px 0;">Prev. Due: ${currencySymbol}${pDue.toFixed(2)}</p>` : ``}${discount > 0 ? `<p style="margin:2px 0;">Discount: -${currencySymbol}${discount.toFixed(2)}</p>` : ``}<h3 style="border-top:1px solid #000; padding-top:5px; margin-top:5px; font-size: 16px;">Total: ${currencySymbol}${grandTotal.toFixed(2)}</h3><p style="margin:2px 0;">Paid: ${currencySymbol}${paidAmt.toFixed(2)}</p><p style="margin:2px 0; font-weight:bold;">Due: ${currencySymbol}${currentDue.toFixed(2)}</p></div><div style="text-align:center; margin-top:20px; font-size:12px; border-top: 1px dashed #000; padding-top: 10px;">*** বিক্রিত মাল ফেরত নেওয়া হয় না ***<br>ধন্যবাদ, আবার আসবেন!</div></div>`; 
...
function viewInvoice(invNo) {
    ...
    let printItemsHtml = `<table style="width:100%; border-collapse: collapse; margin: 15px 0; font-size: 13px; text-align: left;"><tr style="border-bottom: 2px solid #333;"><th style="padding:5px 0;">বিবরণ</th><th>পরিমাণ</th><th style="text-align:right;">মোট (${currencySymbol})</th></tr>`;
    let cartSubTotal = 0;
    if(sale.items && sale.items.length > 0) { sale.items.forEach(item => { let itemTotal = item.qty * item.price; cartSubTotal += itemTotal; printItemsHtml += `<tr style="border-bottom: 1px dotted #ccc;"><td style="padding:5px 0;">${item.name}<br><small>@${currencySymbol}${item.price}</small></td><td>${item.qty}</td><td style="text-align:right;">${itemTotal.toFixed(2)}</td></tr>`; }); } 
    ...
    document.getElementById('reprint-invoice-area').innerHTML = `<div style="font-family: 'Hind Siliguri', sans-serif; color: #000; padding: 10px; max-width: 300px; margin: 0 auto;"><h2 style="text-align:center; margin-bottom: 5px; color: #000; font-size: 22px;">${shopName}</h2><div style="text-align:center; font-size:12px; margin-bottom: 15px;">মোবাইল: ${waNumber || "N/A"}</div><div style="font-size:13px; border-bottom:1px solid #000; padding-bottom:5px; margin-bottom: 5px;"><b>Inv No:</b> ${sale.invoice}<br><b>Date:</b> ${sale.date.split(',')[0]}<br><b>Customer:</b> ${sale.customer}</div>${printItemsHtml}<div style="text-align:right; font-size:14px; margin-top: 10px;"><p style="margin:2px 0;">Subtotal: ${currencySymbol}${cartSubTotal.toFixed(2)}</p>${pDue > 0 ? `<p style="margin:2px 0;">Prev. Due: ${currencySymbol}${pDue.toFixed(2)}</p>` : ``}${discount > 0 ? `<p style="margin:2px 0;">Discount: -${currencySymbol}${discount.toFixed(2)}</p>` : ``}<h3 style="border-top:1px solid #000; padding-top:5px; margin-top:5px; font-size: 16px;">Total: ${currencySymbol}${sale.total.toFixed(2)}</h3><p style="margin:2px 0;">Paid: ${currencySymbol}${sale.paid.toFixed(2)}</p><p style="margin:2px 0; font-weight:bold;">Due: ${currencySymbol}${sale.due.toFixed(2)}</p></div><div style="text-align:center; margin-top:20px; font-size:12px; border-top: 1px dashed #000; padding-top: 10px;">*** বিক্রিত মাল ফেরত নেওয়া হয় না ***<br>ধন্যবাদ, আবার আসবেন!</div></div>`;
...
function renderSalesHistory(filterCus = "All") {
    ...
    sortedSales.forEach(s => {
        if(filterCus === "All" || s.customer === filterCus) {
            ...
            html += `<tr><td><b>${s.invoice}</b><br><small style="color:#64748b;">${timeStr}</small></td><td>${sDate}</td><td><b>${s.customer}</b></td><td style="font-weight:bold; color:#1e293b;">${currencySymbol} ${s.total.toFixed(2)}</td><td style="color:#10b981; font-weight:bold;">${currencySymbol} ${s.paid.toFixed(2)}</td><td style="color:#ef4444; font-weight:bold;">${currencySymbol} ${s.due.toFixed(2)}</td><td class="no-print" style="display:flex; gap:5px;"><button class="btn btn-primary btn-small" onclick="viewInvoice('${s.invoice}')" title="View"><i class="fa-solid fa-eye"></i></button><button class="btn btn-danger btn-small" onclick="deleteInvoice('${s.invoice}')" title="Delete"><i class="fa-solid fa-trash"></i></button></td></tr>`;
        }
    });

    tbody.innerHTML = html || `<tr><td colspan="7" style="text-align:center; padding:20px;">কোনো সেলস ডাটা নেই!</td></tr>`;
    let fts = document.getElementById('filter-total-sell'); if(fts) fts.innerText = `${currencySymbol} ${tS.toFixed(2)}`;
    let ftp = document.getElementById('filter-total-paid'); if(ftp) ftp.innerText = `${currencySymbol} ${tP.toFixed(2)}`;
    let ftd = document.getElementById('filter-total-due'); if(ftd) ftd.innerText = `${currencySymbol} ${tD.toFixed(2)}`;
}
...
function renderDueTable() {
    ...
    debtors.forEach(c => { totalDue += c.due; tbody.innerHTML += `<tr><td onclick="viewCustomerLedger('${c.name}')"><b>${c.name}</b><br><small>${c.phone}</small></td><td>Ledger</td><td style="color:#ef4444; font-weight:bold;">${currencySymbol} ${c.due.toFixed(2)}</td><td class="no-print"><button class="btn btn-warning btn-small" onclick="viewCustomerLedger('${c.name}')">Collect</button></td></tr>`; });
    let tmd = document.getElementById('total-market-due-display'); if(tmd) tmd.innerText = `${currencySymbol} ${totalDue.toFixed(2)}`;
}
...
function updateCostUI() { const costList = document.getElementById('exp-list-body'); if(!costList) return; costList.innerHTML = ''; let total = 0; expensesDB.filter(e => e.type === currentCostTab).forEach(e => { total += e.amount; costList.innerHTML += `<tr><td>${e.cat} <small>(${e.date})</small></td><td style="color:#ef4444;">${currencySymbol} ${e.amount.toFixed(2)}</td><td><button class="btn btn-danger btn-small" onclick="deleteExpense(${e.id})">X</button></td></tr>`; }); let ted = document.getElementById('total-exp-display'); if(ted) ted.innerText = `${currencySymbol} ${total.toFixed(2)}`; }
...
function renderFullAnalytics() {
    ...
    let totDue = customersDB.reduce((sum, cus) => sum + cus.due, 0); let std = document.getElementById('stat-tot-due'); if(std) std.innerText = `${currencySymbol} ${totDue.toFixed(2)}`; 
    let totSale = salesHistoryDB.reduce((sum, sale) => sum + sale.total, 0); let sts = document.getElementById('stat-tot-sale'); if(sts) sts.innerText = `${currencySymbol} ${totSale.toFixed(2)}`; updateAdvChart(); renderHistoricalReport();
}

function renderHistoricalReport() {
    let filterEl = document.getElementById('history-filter-type');
    let filterType = filterEl ? filterEl.value : 'monthly';
    let reportData = {};

    const getFormat = (timestamp) => {
        let d = new Date(timestamp);
        if (filterType === 'daily') return d.toLocaleDateString('en-US', { day: '2-digit', month: 'short', year: 'numeric' });
        if (filterType === 'monthly') return d.toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
        if (filterType === 'yearly') return d.getFullYear().toString();
    };

    const initGroup = (key) => { if(!reportData[key]) reportData[key] = { sell: 0, profit: 0, cost: 0, newDue: 0, dueCol: 0, timestamp: 0 }; };

    salesHistoryDB.forEach(s => { let key = getFormat(s.rawDate || s.date); initGroup(key); reportData[key].sell += s.total; reportData[key].profit += (s.profit !== undefined ? s.profit : 0); reportData[key].newDue += s.due; reportData[key].timestamp = s.rawDate || new Date(s.date).getTime(); });
    expensesDB.forEach(e => { let key = getFormat(e.rawDate || e.date); initGroup(key); reportData[key].cost += e.amount; if (reportData[key].timestamp === 0) reportData[key].timestamp = e.rawDate || new Date(e.date).getTime(); });
    dueCollectionHistoryDB.forEach(d => { let key = getFormat(d.rawDate || d.date); initGroup(key); reportData[key].dueCol += d.amount; if (reportData[key].timestamp === 0) reportData[key].timestamp = d.rawDate || new Date(d.date).getTime(); });

    let sortedKeys = Object.keys(reportData).sort((a, b) => reportData[b].timestamp - reportData[a].timestamp);
    let tbody = document.getElementById('historical-report-body'); if(!tbody) return;

    let html = '';
    sortedKeys.forEach(key => {
        let d = reportData[key]; let netProfit = d.profit - d.cost; let profitColor = netProfit >= 0 ? '#10b981' : '#ef4444'; let profitIcon = netProfit >= 0 ? '<i class="fa-solid fa-arrow-trend-up"></i>' : '<i class="fa-solid fa-arrow-trend-down"></i>';
        html += `<tr><td style="font-weight: bold; color: #1e293b; font-size: 15px;">${key}</td><td style="color: #3b82f6; font-weight: bold;">${currencySymbol} ${d.sell.toFixed(2)}</td><td style="color: #ef4444;">${currencySymbol} ${d.cost.toFixed(2)}</td><td style="color: #f59e0b;">${currencySymbol} ${d.newDue.toFixed(2)}</td><td style="color: #0ea5e9;">${currencySymbol} ${d.dueCol.toFixed(2)}</td><td style="color: ${profitColor}; font-weight: bold; font-size: 16px;">${profitIcon} ${currencySymbol} ${netProfit.toFixed(2)}</td></tr>`;
    });
    if (sortedKeys.length === 0) html = `<tr><td colspan="6" style="text-align: center; padding: 20px; color: #64748b;">কোনো ডেটা পাওয়া যায়নি!</td></tr>`;
...
function updateAdvChart() {
    ...
    advAnalyticsChart = new Chart(ctx, { type: 'line', data: { labels: ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'], datasets: [{ label: `Sales (${currencySymbol})`, data: salesData, borderColor: '#3b82f6', backgroundColor: 'rgba(59, 130, 246, 0.1)', fill: true, tension: 0.3 }, { label: `Profit (${currencySymbol})`, data: profitData, borderColor: '#10b981', backgroundColor: 'rgba(16, 185, 129, 0.1)', fill: true, tension: 0.3 }, { label: `Cost (${currencySymbol})`, data: costData, borderColor: '#ef4444', backgroundColor: 'rgba(239, 68, 68, 0.1)', fill: true, tension: 0.3 }] }, options: { responsive: true, maintainAspectRatio: false } });
}

// ================= SETTINGS SAVE & LOAD =================
function saveSettings() {
    if(!currentShopID) return alert("Logout hoye gechen!");
    let btn = event.currentTarget; let originalText = btn.innerHTML; btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Saving...'; btn.disabled = true;

    const settings = {
        address: document.getElementById('setting-shop-address').value,
        currency: document.getElementById('setting-currency').value,
        tax: parseFloat(document.getElementById('setting-tax-percent').value) || 0,
        header: document.getElementById('setting-inv-header').value,
        footer: document.getElementById('setting-inv-footer').value,
        printSize: document.getElementById('setting-print-size').value,
        showDue: document.getElementById('setting-show-due').checked,
        payMfs: document.getElementById('setting-pay-mfs').checked,
        payCard: document.getElementById('setting-pay-card').checked,
        language: document.getElementById('setting-language').value,
        darkMode: document.getElementById('setting-dark-mode').checked,
        waNumber: document.getElementById('setting-wa-number').value,
        lowStockLimit: parseInt(document.getElementById('setting-low-stock-limit').value) || 5,
        barcodeSupport: document.getElementById('setting-barcode-support').checked,
        soundEffect: document.getElementById('setting-sound-effect').checked,
    ...
    if (!currentShopID.startsWith("TRIAL-")) {
        db.collection('shops').doc(currentShopID).set({ settings: settings, waNumber: waNumber }, { merge: true }).then(() => {
            btn.innerHTML = originalText; btn.disabled = false; alert("✅ Settings successfully saved!"); switchDashboardView('daily'); renderAllTables();
        }).catch(e => { btn.innerHTML = originalText; btn.disabled = false; alert("❌ Cloud save fail! (Saved locally)"); });
    } else { btn.innerHTML = originalText; btn.disabled = false; alert("✅ Settings saved locally (Trial Mode)"); switchDashboardView('daily'); renderAllTables(); }
}

function loadSettings() {
    if(!currentShopID) return;
    let s = localStorage.getItem(`pos_${currentShopID}_settings`);
    if(s) {
        let settings = JSON.parse(s);
        currencySymbol = settings.currency || "৳";
        if(document.getElementById('setting-shop-address')) document.getElementById('setting-shop-address').value = settings.address || "";
        if(document.getElementById('setting-currency')) document.getElementById('setting-currency').value = settings.currency || "৳";
        if(document.getElementById('setting-tax-percent')) document.getElementById('setting-tax-percent').value = settings.tax || 0;
        if(document.getElementById('setting-inv-header')) document.getElementById('setting-inv-header').value = settings.header || "বিসমিল্লাহির রাহমানির রাহিম";
        if(document.getElementById('setting-inv-footer')) document.getElementById('setting-inv-footer').value = settings.footer || "*** বিক্রিত মাল ফেরত হয় না ***";
        ...
        if(document.getElementById('setting-wa-number')) document.getElementById('setting-wa-number').value = settings.waNumber || "";
        ...
        if(document.getElementById('setting-language')) document.getElementById('setting-language').value = settings.language || "en-bn";
        if(document.getElementById('setting-show-due')) document.getElementById('setting-show-due').checked = settings.showDue !== false;
        if(document.getElementById('setting-pay-mfs')) document.getElementById('setting-pay-mfs').checked = settings.payMfs !== false;
        if(document.getElementById('setting-pay-card')) document.getElementById('setting-pay-card').checked = settings.payCard === true;
        if(document.getElementById('setting-dark-mode')) document.getElementById('setting-dark-mode').checked = settings.darkMode === true;
        if(document.getElementById('setting-barcode-support')) document.getElementById('setting-barcode-support').checked = settings.barcodeSupport !== false;
        if(document.getElementById('setting-sound-effect')) document.getElementById('setting-sound-effect').checked = settings.soundEffect !== false;
        if(document.getElementById('setting-auto-sync')) document.getElementById('setting-auto-sync').checked = settings.autoSync !== false;
    }
}

function factoryResetApp() {
    if(prompt("সতর্কতা! সমস্ত ডেটা মুছে যাবে। আপনি যদি নিশ্চিত হন, তবে নিচে 'DELETE' লিখে OK করুন:") === 'DELETE') {
        customersDB = []; productsDB = []; salesHistoryDB = []; dueCollectionHistoryDB = []; expensesDB = [];
        saveData(); renderAllTables(); alert("🚨 সিস্টেম ফ্যাক্টরি রিসেট সম্পন্ন হয়েছে!"); window.location.reload();
    }
}
